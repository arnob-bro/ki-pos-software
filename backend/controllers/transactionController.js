...
class TransactionController {
	...
	async addTransaction(data, currentUser) {
		try {
			console.log(
				"DEBUG: TransactionController.addTransaction called with:",
			...
			if (
				...
				throw new Error(
					"Missing required transaction fields: user_id, payment_method, and total_amount are required"
				);
			}
			...
			for (const item of processedData.items) {
				...
				if (item.quantity <= 0 || item.unit_price <= 0) {
					throw new Error(
						"Item quantity and unit_price must be positive numbers"
					);
				}
			}
