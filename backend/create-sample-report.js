...
async function createSampleReport() {
  ...
  try {
    ...
    insertStmt.run(reportId, 'x_report', 'test-user', JSON.stringify(sampleXReport));
    ...
    insertStmt.run(zReportId, 'z_report', 'test-user', JSON.stringify(sampleZReport));
    
    console.log('✅ Sample Z report created with ID:', zReportId);
    
    // Verify the reports were created
    const reports = db.prepare('SELECT id, type, generated_at FROM generated_reports').all();
    console.log('\n📊 Reports in database:');
    ...
    console.log('You can now test the download functionality in the application.');
