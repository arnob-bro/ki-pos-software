...
try {
  ...
  `
	...
	if (corruptedReports.length > 0) {
		console.log(
			`⚠️  Found ${corruptedReports.length} reports with missing data:`
		);
		corruptedReports.forEach((report) => {
			console.log(
				`   - ${report.id} (${report.type}) - ${report.generated_at}`
			);
		});
  ...
  `
	...
	testReports.forEach((report, index) => {
		try {
			...
			console.log(
				`${index + 1}. ${report.id} (${report.type}): ❌ Invalid JSON - ${
					error.message
				}`
			);
		}
