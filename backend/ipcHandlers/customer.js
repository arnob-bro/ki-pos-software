...
module.exports = function registerCustomerHandlers(ipcMain, db) {
    ...
    ipcMain.handle("customers:add", async (event, customerData) => {
        return await customerController.addCustomer(customerData)
    })

    ipcMain.handle("customers:update", async (event, customerData) => {
    ...
    ipcMain.handle("customers:assignLoyaltyTier", async (event, id, tier) => {
        return await customerController.assignLoyaltyTier(id, tier)
    })
