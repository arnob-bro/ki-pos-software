...
class ProductService {
	...
	async addProduct(product, currentUser) {
		...
		const stmt = this.db.prepare(
			"INSERT INTO products (id, name, category_id, barcode, price, vat_rate, stock_quantity) VALUES (?, ?, ?, ?, ?, ?, ?)"
		);
	...
	async updateProduct(product, currentUser) {
		...
		const stmt = this.db.prepare(
			"UPDATE products SET name = ?, category_id = ?, barcode = ?, price = ?, vat_rate = ?, stock_quantity = ? WHERE id = ?"
		);
