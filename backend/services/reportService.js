...
class ReportService {
	...
	async generateXReport(date, userId) {
		try {
			...
			const userCheck = this.db
				.prepare("SELECT id FROM users WHERE id = ?")
				.get(userId);
			if (!userCheck) {
				throw new Error(
					`Failed to generate X report: userId '${userId}' does not exist in users table.`
				);
			}
      ...
      `);
			...
			const topProducts = productStmt
				.all(date)
				.map((row) => ({ name: row.name, qty: row.total_qty }));
	...
	async generateZReport(date, userId) {
		try {
			...
			if (!userCheck) {
				throw new Error(
					`Failed to generate Z report: userId '${userId}' does not exist in users table.`
				);
			}
	...
	async getGeneratedReports(userId, page = 1, limit = 20) {
        ...
        `);
			...
			const countStmt = this.db.prepare(
				"SELECT COUNT(*) as total FROM generated_reports"
			);
	...
	async exportGoBDData(startDate, endDate) {
      ...
      `);
			...
			transactions.forEach((tx) => {
				...
				tx.vat_rate =
					txItems.length > 0
						? Math.max(...txItems.map((i) => i.vat_rate || 0))
						: "";
			...
			items.forEach((i) => {
				...
				if (i.vat_amount == null) {
					i.vat_amount = (
						(i.unit_price * i.quantity * (i.vat_rate || 0)) /
						100
					).toFixed(2);
				}
			});
			...
			const totalNet = items.reduce(
				(sum, i) =>
					sum + (parseFloat(i.unit_price) || 0) * (parseFloat(i.quantity) || 0),
				0
			);
			const totalVAT = items.reduce(
				(sum, i) => sum + (parseFloat(i.vat_amount) || 0),
				0
			);
			const avgTxnValue = transactions.length
				? totalGross / transactions.length
				: 0;
			const highestTxn = Math.max(
				...transactions.map((t) => parseFloat(t.total_amount) || 0),
				0
			);
			const lowestTxn = Math.min(
				...transactions.map((t) => parseFloat(t.total_amount) || 0),
				0
			);
			...
			const otherTxns = transactions.filter(
				(t) => t.payment_method === "other"
			);
			const cashTotal = cashTxns.reduce(
				(sum, t) => sum + (parseFloat(t.total_amount) || 0),
				0
			);
			...
			const topProduct = sortedProducts[0]
				? `${sortedProducts[0][0]} (${sortedProducts[0][1]} units)`
				: "";
			const secondProduct = sortedProducts[1]
				? `${sortedProducts[1][0]} (${sortedProducts[1][1]} units)`
				: "";

			// VAT summary
			const vatRate = items.length > 0 ? items[0].vat_rate || 19 : 19;
			// Transaction statistics
			const avgItemsPerTxn = transactions.length
				? totalItemsSold / transactions.length
				: 0;
			...
			if (Object.keys(hourCounts).length > 0) {
				const peak = Object.entries(hourCounts).reduce((a, b) =>
					a[1] > b[1] ? a : b
				);
				const slow = Object.entries(hourCounts).reduce((a, b) =>
					a[1] < b[1] ? a : b
				);
				peakHour = `${String(peak[0]).padStart(2, "0")}:00-${String(
					Number(peak[0]) + 1
				).padStart(2, "0")}:00 (${peak[1]} transactions)`;
				slowestHour = `${String(slow[0]).padStart(2, "0")}:00-${String(
					Number(slow[0]) + 1
				).padStart(2, "0")}:00 (${slow[1]} transactions)`;
			}
			...
			const exportResult = await exportGoBD(
				summary,
				transactions,
				items,
				{},
	...
	async getReportStats(userId) {
        ...
        `);
				...
				totalStmt = this.db.prepare(
					"SELECT COUNT(*) as count FROM generated_reports WHERE user_id = ?"
				);
