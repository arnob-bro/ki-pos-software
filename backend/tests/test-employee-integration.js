...
async function testEmployeeIntegration() {
	...
	try {
		...
		const statusUpdated = await employeeService.updateEmployeeStatus(
			addedEmployee.id,
			"suspended"
		);
		...
		const employeePermissions = await employeeService.getEmployeePermissions(
			addedEmployee.id
		);
		console.log(
			"Employee permissions:",
			employeePermissions.map((p) => p.code)
		);
