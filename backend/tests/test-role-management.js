...
async function testRoleManagement() {
	...
	try {
		...
		console.log(
			"Role permissions:",
			addedRole.permissions.map((p) => p.code)
		);
		...
		const updatedRole = await employeeService.updateRole(
			addedRole.id,
			updateData
		);
		...
		console.log(
			"Updated permissions:",
			updatedRole.permissions.map((p) => p.code)
		);
		...
		console.log(
			"Roles after deletion:",
			rolesAfterDelete.map((r) => r.name)
		);
