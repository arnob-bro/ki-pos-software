...
const CompanyProfile = () => {
	...
	const handleSubmit = (e) => {
		...
		fetch("http://localhost:4000/api/company-profile", {
			...
			.then(() =>
				alert(
					t("Profile updated successfully!", "Profil erfolgreich aktualisiert!")
				)
			)
			.catch((err) => {
				console.error(err);
				alert(
					t(
						...
						"Fehler beim Aktualisieren des Firmenprofils"
					)
				);
			});
	};
			...
			{showModal && (
				<div className='modal-overlay' onClick={() => setShowModal(false)}>
					<div className='modal' onClick={(e) => e.stopPropagation()}>
						...
						<h3>
							{t("📘 GoBD/GDPdU Compliance", "📘 GoBD/GDPdU-Konformität")}
						</h3>
						<ul>
							<li>
								{t(
