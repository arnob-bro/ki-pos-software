...
const Reports = () => {
			...
			if (stored) {
				try {
					...
					const checks = await Promise.all(
						files.map((f) => window.posAPI.fileExists(f))
					);
	...
	const checkZReportExists = async () => {
		try {
			...
			const result = await window.posAPI.checkZReportExists(
				selectedDate,
				userId
			);
	...
	const generateZReport = async () => {
		try {
			...
			if (error.message.includes("already exists for this date")) {
				setMessage(
					"Z Report already exists for this date. Z reports can only be generated once per day."
				);
			} else {
	...
	const exportGoBD = async () => {
		...
		setMessage("");
		try {
			const result = await window.posAPI.exportGoBD(
			...
			if (result.success && files.length > 0) {
				...
				setMessage(
					"GoBD export completed successfully, but file path is missing."
				);
			} else {
				setMessage("Error: " + result.message);
			}
	...
	const fetchCategoryData = async () => {
		...
		setCategoryError("");
		try {
			const res = await window.posAPI.reports_salesByCategory(
				managerDateRange.startDate,
				managerDateRange.endDate
			);
	...
	const fetchTimeData = async () => {
		...
		setTimeError("");
		try {
			const res = await window.posAPI.reports_salesByTime(
				managerDateRange.startDate,
				managerDateRange.endDate,
				timeInterval
			);
	...
	const fetchOperatorData = async () => {
		...
		setOperatorError("");
		try {
			const res = await window.posAPI.reports_salesByOperator(
				managerDateRange.startDate,
				managerDateRange.endDate
			);
	...
	const fetchTaxData = async () => {
		...
		setTaxError("");
		try {
			const res = await window.posAPI.reports_taxBreakdown(
				managerDateRange.startDate,
				managerDateRange.endDate
			);
		...
		<div className='reports-container'>
			...
			<div className='reports-content'>
				<h1 className='reports-title'>
					{t("📊 Reports Section", "📊 Berichtsbereich")}
				</h1>
						...
						<div className='stats-grid'>
							<div className='stat-item'>
								...
								<span className='stat-label'>
									{t("Today's Reports", "Heutige Berichte")}
								</span>
							</div>
							<div className='stat-item'>
							...
							<div className='stat-item'>
								...
								<span className='stat-label'>
									{t("Total Reports", "Gesamte Berichte")}
								</span>
							</div>
						</div>
					</div>
				...
				<div className='reports-section'>
					...
					<p>
						{t(
							"Access all daily X and Z reports generated by the POS system.",
							"Zugang zu allen täglich generierten X- und Z-Berichten des POS-Systems."
						)}
					</p>
					...
					<div className='button-group'>
						...
						>
							{loading
								...
								: t("Generate X Report", "X-Bericht generieren")}
						</button>
						<button
						...
						>
							{loading
								? t("Generating...", "Generiere...")
								: zReportExists
								? t("Z Report Already Generated", "Z-Bericht bereits generiert")
								: t("Generate Z Report", "Z-Bericht generieren")}
						</button>
					</div>
				</div>
				...
				{user?.role_id !== 2 && (
					<div className='reports-section'>
						...
						<p>
							{t(
								"Generate export files compliant with German tax audit standards.",
								"Erstellen Sie Exportdateien gemäß den deutschen GoBD-Standards."
							)}
						</p>
						<button
							className='reports-button'
				...
				<div className='reports-section'>
					...
					<p>
						{t(
							"Download archived reports and invoices in PDF format.",
							"Laden Sie archivierte Berichte und Rechnungen im PDF-Format herunter."
						)}
					</p>
					...
					{reports.length > 0 ? (
						...
						<p>
							{t("No reports generated yet.", "Noch keine Berichte generiert.")}
						</p>
					)}
				</div>
					...
					<div className='reports-section manager-reports'>
						...
						<div className='manager-date-range'>
							<label>
								...
								<input
									type='date'
									value={managerDateRange.startDate}
									onChange={(e) =>
										setManagerDateRange((d) => ({
											...d,
							...
							<label>
								...
								<input
									type='date'
									value={managerDateRange.endDate}
									onChange={(e) =>
										setManagerDateRange((d) => ({
											...d,
						...
						<div style={{ margin: "18px 0" }}>
							<label style={{ fontWeight: 500 }}>
								{t("Select Report Type:", "Berichtstyp auswählen:")}{" "}
							</label>
							<select
								value={managerReportType}
								onChange={(e) => setManagerReportType(e.target.value)}
								style={{ marginLeft: 8, padding: "8px 8px" }}
							>
								<option value='category'>
									{t("Sales by Category", "Umsatz nach Kategorie")}
								</option>
								<option value='time'>
									{t("Sales by Time", "Umsatz nach Zeit")}
								</option>
								<option value='operator'>
									{t("Sales by Operator", "Umsatz nach Bediener")}
								</option>
								<option value='tax'>
									{t("Tax/VAT Breakdown", "Steuer-/MwSt.-Aufschlüsselung")}
								</option>
							</select>
						...
						<div className='manager-report-block'>
									...
									<table className='manager-table'>
										<thead>
											<tr>
												...
												<th>{t("Total Sales", "Gesamtumsatz")}</th>
												<th>{t("Quantity Sold", "Verkaufte Menge")}</th>
											</tr>
										</thead>
										<tbody>
											{categoryData.length === 0 ? (
												<tr>
													<td colSpan='3'>{t("No data", "Keine Daten")}</td>
												</tr>
											) : (
							...
							{managerReportType === "time" &&
								(timeLoading ? (
									...
									<table className='manager-table'>
										<thead>
											<tr>
												<th>{t("Interval", "Intervall")}</th>
												<th>{t("Total Sales", "Gesamtumsatz")}</th>
												<th>{t("Transactions", "Transaktionen")}</th>
											</tr>
										</thead>
										<tbody>
											{timeData.length === 0 ? (
							...
							{managerReportType === "operator" &&
								(operatorLoading ? (
									...
									<div className='error'>{operatorError}</div>
								) : (
									<table className='manager-table'>
										<thead>
											<tr>
							...
							{managerReportType === "tax" &&
								(taxLoading ? (
									...
									<div className='error'>{taxError}</div>
								) : (
									<table className='manager-table'>
										<thead>
											<tr>
												<th>{t("Tax Rate", "Steuersatz")}</th>
												<th>{t("Net Sales", "Nettoumsatz")}</th>
												<th>{t("Tax Amount", "Steuerbetrag")}</th>
												<th>{t("Gross Sales", "Bruttoumsatz")}</th>
											</tr>
										</thead>
										<tbody>
											{taxData.length === 0 ? (
												<tr>
													<td colSpan='4'>{t("No data", "Keine Daten")}</td>
												</tr>
											) : (
												taxData.map((row, i) => (
													<tr key={i}>
														<td>{row.tax_label}</td>
														<td>{row.net_sales?.toFixed(2)}</td>
					...
					<div className='modal-overlay'>
						<div className='modal'>
							<h2 style={{ marginTop: 0, marginBottom: 8 }}>
								Export GoBD-compliant
							</h2>
							<div
								style={{ color: "#666", marginBottom: 18, fontSize: "15px" }}
							>
								Generate a legally compliant export for your selected date
								range.
							</div>
